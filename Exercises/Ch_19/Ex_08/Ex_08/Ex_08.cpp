...
public:
	...
	void deallocate(T* elems) { free(elems); }
	void construct(T* dest, const T& orig) { new(dest) T{ orig }; }
	void destroy(T* elem) { elem->~T(); }
};

int main()
try {
	Vector<int, Allocator<int>> vi1(10);
	vi1.push_back(1);
	...
	Vector<int, Allocator<int>> vi3{ vi1 };
