...
int count_chars(Document& doc, const std::string& ws_chars)
{
	int count = 0;
	for (auto p = doc.begin(); p != doc.end(); ++p) {
		if (ws_chars.find(*p) == std::string::npos)
			++count;
	}
	return count;
}
...
try {
	...
	iss >> my_doc;
	print(my_doc);
	std::string ws_chars{ "aeiou" };
	std::cout << "Number of non-vowel characters in document: " << count_chars(my_doc, ws_chars) << '\n';
}
