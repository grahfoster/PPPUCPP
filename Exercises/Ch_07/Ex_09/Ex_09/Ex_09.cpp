...
public:
	...
	string name;
	double value;
	Variable(string n, double v) :constant(0), name(n), value(v) { }
...
double Symbol_table::get(string s)	// get the value of a variable by name
{
...
void Symbol_table::set(string s, double d)	// set the value of an existing variable
{
	for (int i = 0; i < names.size(); ++i)
		if (names[i].name == s) {
...
double fact()
{
	...
	while (true) {
		if (t.kind == '!') {
			int left_temp;
			try { left_temp = narrow_cast<int>(left); }
			catch (exception& e) { error("attempted ! on a double"); }	// throw an error if the user gave a double
			if (left_temp == 0)	left = 1;	// "0!" is equal to 1
			else {
				for (int i = 1; i < int(left); ++i)	// calculate factorial
					left_temp *= i;
				left = left_temp;
			}
			t = ts.get();
