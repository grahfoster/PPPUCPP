...
try {
	...
	for (int i = 0; i < num_rows; ++i)
		for (int j = 0; j < num_rows; ++j) {
			rects.push_back(new Graph_lib::Rectangle{ {i * rect_width, j * rect_width }, rect_width, rect_width });
			rects[rects.size() - 1].set_color(Color::black);
			rects[rects.size() - 1].set_fill_color(fl_rgb_color(i * num_rows, j * num_rows, i * j));
