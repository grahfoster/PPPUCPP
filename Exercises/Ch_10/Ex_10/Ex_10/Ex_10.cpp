/*
	...
	may declare their own constants and variables, as well as assign new values to variables. Additionally,
	users may assign their own input and output streams.
...
void calculate()
{
	...
	while ((!ts.get_ifstream().is_open() && cin) || ts.get_ifstream()) try {
		...
		if (!ts.get_ifstream().is_open()) {
			...
			ts.unget(t);
			if (!ts.get_ofstream().is_open())
				cout << result << statement(cin) << endl;
			else
				ts.get_ofstream() << result << statement(cin) << endl;
		}
		else {
			...
			ts.unget(t);
			if (!ts.get_ofstream().is_open())
				cout << result << statement(ts.get_ifstream()) << endl;
			else
				ts.get_ofstream() << result << statement(ts.get_ifstream()) << endl;
		}
	}
