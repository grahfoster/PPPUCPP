...
Token Token_stream::get(istream& stream)
{
	...
	case 'M':
	{
		string numerals;
		numerals += ch;
		while (stream.get(ch) && is_numeral(ch)) numerals += ch;
		stream.unget();
		Roman_int roman_int{ numerals };
		return Token(number, roman_int.to_int());
	}
	default:
...
void print_help()
{
	cout << "Welcome to our simple calculator.\n"
		<< "Please enter expressions using Roman numerals.\n"
		<< "Available operators: +, -, *, /, %, !.\n"
...
void calculate(istream & stream)
{
	while (stream) try {
		...
		ts.unget(t);
		cout << result << to_numerals(statement(stream)) << endl;
	}
	catch (exception & e) {
		cerr << e.what() << endl;
		stream.unget();
