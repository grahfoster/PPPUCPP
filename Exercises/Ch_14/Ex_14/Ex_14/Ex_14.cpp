...
void Binary_tree::put_label(int i, int j, const string& label)
{
	const int node_height = height / num_levels;
	const int node_radius = node_height / 2 / (i + 1);
	const int padding = node_radius / 4;
	const int font_size = 32 / (i + 1);

	labels.push_back(new Text{ { nodes[i][j]->center().x - node_radius + padding,
		nodes[i][j]->center().y }, label });
	labels.back()->set_font_size(font_size);
}
...
try {
	...
	b_tree.put_label(0, 0, "label1");
	b_tree.put_label(1, 1, "label2");
	b_tree.put_label(3, 4, "label3");
