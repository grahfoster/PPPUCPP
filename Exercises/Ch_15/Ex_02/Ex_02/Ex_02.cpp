...
Funct::Funct(Fct f, double r1, double r2, Point o, int c, double xs, double ys) :
	count{ c }
{
	reset_all(f, r1, r2, o, xs, ys);
}

void Funct::reset_all(Fct f, double r1, double r2, Point o, double xs, double ys)
{
	fct = f;
	range1 = r1;
	range2 = r2;
	...
	if (number_of_points() == count)
		for (int i = 0; i < count; ++i) {
			set_point(i, Point(origin.x + int(r * xscale), origin.y - int(fct(r) * yscale)));
			r += dist;
		}
	else
		for (int i = 0; i < count; ++i) {
			add(Point(origin.x + int(r * xscale), origin.y - int(fct(r) * yscale)));
			r += dist;
		}
}
}
...
try {
	...
	Funct funct{ cos, -10, 10, { s_win.x_max() / 2, s_win.y_max() / 2 } };
	funct.set_color(Color::black);

	funct.reset_all(sin, -25, 25, { s_win.x_max() / 2, s_win.y_max() / 4 }, 10, 10);
