...
try {
	...
	Bar_graph b_graph{ origin, width, height, min_value, max_value, bar_datas, "Distribution of Heights" };
	b_graph.set_color(Color::black);

	const int x_length = width + 25;

	Axis x_axis{ Axis::Orientation::x, origin, x_length, 0, "Height (cm)" };
	x_axis.set_color(Color::black);
	x_axis.label.move(origin.x + x_length / 6, 50);

	const int y_length = height + 25;

	Axis y_axis{ Axis::Orientation::y, origin, y_length, 0, "Number" };
